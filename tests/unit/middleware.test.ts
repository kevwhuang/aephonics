...
describe('onRequest', () => {
    ...
    test('lets a throw on the /500 path propagate', async () => {
        const { context, rewrite } = createContext('/500');
    ...
    test('passes page 404 responses through without rewriting', async () => {
        const { context, rewrite } = createContext('/');
        const missing = new Response('missing', { status: 404 });

        const next: MiddlewareNext = vi.fn(async () => missing);
    ...
    test('rewrites page responses with status 500 or above to /500', async () => {
        ...
        expect(rewrite).toHaveBeenCalledExactlyOnceWith('/500');
    });

    test('passes api success responses through untouched', async () => {
        const { context, rewrite } = createContext('/api/anything');
        const success = new Response('ok', { status: 200 });

        const next: MiddlewareNext = vi.fn(async () => success);
